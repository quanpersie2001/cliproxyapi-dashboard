...
export function OwnerBadge({ ownerUsername, isOwn }: OwnerBadgeProps) {
  if (isOwn) {
    return (
      <Badge tone="info" className="rounded-sm px-2 py-0.5 text-[11px]">
        You
      </Badge>
    );
  }
  ...
  if (ownerUsername) {
    return (
      <Badge className="rounded-sm px-2 py-0.5 text-[11px]">
        {ownerUsername}
      </Badge>
    );
  }
  ...
  return (
    <Badge className="rounded-sm px-2 py-0.5 text-[11px] text-[var(--text-muted)]">
      Team
    </Badge>
  );
}
