...
|---|---|---|
| `dashboard/package.json`, `dashboard/Dockerfile`, `dashboard/entrypoint.sh` | High | Story 1 owns the runtime packaging shape; later stories may consume it but not redesign it. |
| `dashboard/src/usage-collector/core/orchestrator.ts` and `core/one-shot-orchestrator.ts` | Medium | Preserve the approved one-shot contract; Story 2 may compose it into a worker but should not reopen Phase 2 ingestion semantics without explicit need. |
| `dashboard/src/usage-collector/repositories/*` and any new worker control repository | High | Story 2 owns wake/heartbeat/leadership state; keep DB-visible control logic centralized. |
| `dashboard/src/app/api/usage/collect/route.ts` | High | Story 3 owns the route rewrite and must preserve auth/origin validation plus the `200`/`202` refresh contract. |
| `dashboard/src/usage-collector/repositories/usage-record-repository.ts`, `dashboard/src/usage-collector/core/process-service.ts`, `dashboard/src/lib/__tests__/usage-history.test.ts` | High | Story 4 ownership-persistence repair must preserve the approved `usage_records` read model and non-admin visibility semantics. |
| `dashboard/src/lib/env.ts` | Medium | Story 1 or Story 2 may add runtime envs, but keep validation additive and aligned with the final packaging shape. |
