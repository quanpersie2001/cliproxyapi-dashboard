...
- `P2` Leadership lock lifetime is still tied to a 120s transaction timeout, which can self-abort long backlog drains under load (`apps/dashboard/src/usage-collector/infra/leader-lock.ts`).
- `P2` Inbox finalization still does not assert affected-row count before reporting processed success (`apps/dashboard/src/usage-collector/repositories/inbox-repository.ts`).
- `P2` Worker sleep shutdown path lacks direct abort-after-start proof (`apps/dashboard/src/usage-collector/runner.ts`, `apps/dashboard/src/usage-collector/__tests__/worker-runner.test.ts`).
- `P3` The async trigger contract remains semantically ambiguous between `200 queued` and `202 accepted` (`apps/dashboard/src/app/api/usage/collect/route.ts`).
